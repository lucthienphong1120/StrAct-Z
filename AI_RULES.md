...
>
> Mỗi khi thay đổi một logic cốt lõi nào đó, hãy cập nhật nó vào `AI_RULES.md` để "bộ nhớ" này luôn luôn mới nhất! Đồng thời cập nhật tiến độ hoàn thành các tính năng vào `PLAN.md`, rồi thực hiện `git add`, `git commit`, `git push` để đồng bộ hóa kho mã nguồn lên GitHub immediately!
...
- **Walking**: Distance x0.55 / Pace x1.7.
- **Running**: Distance x1.0 / Pace x1.0.
...
- **Feature: Explicit Card Save Buttons & Real-Time Scheduler Display Update**:
  ...
  - Allowed `scheduleEnabled` toggle checkbox to still immediately commit scheduler activation status to the database.
  - Fixed a race condition where time slot 2 was lost on page load/refresh because `loadSchedule()` was executed before `loadStats()` populated `window.userRole = 'vip'`. Now `loadStats` is awaited sequentially first in `loadDashboard`.
  - Implemented real-time scheduler pending/next-run display updates (`refreshScheduleDisplayFromUI`) responding immediately to custom time, date picker inputs, and scheduler card changes without requiring database saving or page reloads.
...
- **Feature: Pending Auto Scheduler on Custom Time & Future Date Allowance**:
  ...
  - Updated validator `validation.js` to allow future dates for `target_date` (up to the role's maximum range limit) only when `custom_time_enabled` is active.
  - Dynamically updated the date picker `max` constraint on the UI based on the Custom Time toggle state.
  - Displayed the pending custom time notice next to the auto-run text on the dashboard.
...
- **Feature: Customized Activity Multipliers**:
  ...
  - Walk: Distance x0.55 / Pace x1.7.
  - Cycling (Ride): Distance x2.3 / Pace x0.45.

### v1.52.4 (2026-06-03)
- **Feature: Synchronized Multiplier Baseline**:
  - Re-anchored activity multipliers directly to Running (Run = 1.0x). The Min/Max Pace and Min/Max Distance inputs in Settings now directly represent running values.
  - Walk: Distance x0.55 / Pace x1.75.
  - Cycling (Ride): Distance x2.4 / Pace x0.4.
...
### v1.39.0 (2026-05-09)
- **Map Integration**: Replaced simplified circular highlights with actual geographical boundaries (polygons) for all 12 urban districts of Hanoi using GeoJSON data.
...
### v1.30.0 (2026-05-09)
- **Centralized Config**: Chuyển toàn bộ các giới hạn validate và đặc quyền VIP vào hệ thống cấu hình tập trung (`src/config/limits.js`).
- **Dynamic UI Limits**: Giao diện tự động cập nhật các ràng buộc (min/max), thông báo lỗi và **Tooltips** dựa trên vai trò của người dùng (Normal/VIP) lấy từ server.
...
- **Tooltip Updates**: Refined tooltips for Weather and Red Lights to accurately describe HR behavior (heat stress and pause decay).
...
### v1.22.0
- **Smart Heart Rate Logic**: Simplified HR settings by removing "Min HR". HR ranges are now automatically calculated based on your **Max Heart Rate (MHR)** and activity type:
  - **Walk**: 50% - 60% MHR
  - **Ride**: 60% - 70% MHR
  - **Run**: 70% - 85% MHR
- **HR Tooltips**: Added formula `MHR = 220 - Age` and intensity zone info to tooltips.
- **Bug Fixes**: Unified internal HR calculation logic.

### v1.20.0
- **Activity Type Multipliers**: Introduced automatic distance scaling based on the sport type:
...
### v1.19.0
- **Refined Weighting Logic**: Transitioned from distance-decay to a precise additive boost system based on circle coverage.
...
- **Weighted District Selection**: The generation engine now uses your configured Map Areas to prioritize nearby districts, making random routes more localized.
...
- **User Roles (VIP vs Normal)**: Introduced role-based restrictions. Default users are `normal`, with limits on district span (max 2) and daily scheduled activities (max 2). `vip` users can extend these limits.
- **Backend Role Enforcement**: Added server-side validation to prevent non-VIP users from exceeding their quota via API.
...
- **OSRM description note** added below the toggle explaining what it does and its fallback behavior.

### v1.15.0
- **Documentation Split**: Documentation split into `docs/ARCHITECTURE.md`, `docs/SETUP_GUIDE.md`, `docs/USER_GUIDE.md`.
- **Bug Fix**: Fixed delete button silently failing when browser pop-up dialogs were blocked.
- **Account Settings**: Added Account Settings card for changing password.
- **Hà Đông default**: Hà Đông district set as ON by default; auto-migration script for existing configs.
- **Version Badge**: Version badge displayed in UI header.

### v1.14.0
- **Multi-Tenant Architecture**: Full multi-tenant architecture: per-user database, config, Strava tokens, and schedulers.
- **Security**: JWT + bcrypt authentication; IP rate-limiting on login (brute-force protection).
- **Setup Wizard**: First-time Admin registration wizard at `/register.html`.
- **Districts Expansion**: Added 4 new districts: Long Biên, Hà Đông, Bắc Từ Liêm, Nam Từ Liêm (3-column layout).
- **Simulations**: Weather Simulation & Red Light Stops toggles added to UI.
- **Scheduler**: Multi-user Scheduler with independent cron jobs per account.
