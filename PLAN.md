...
+ [ ] kiểm tra logic gen hoạt động hiện tại có tính chia chi tiết đơn vị nhỏ nhất là gì, có đang theo phường/xã trong quận ko? tôi muốn xử lý tăng độ ưu tiên với các khu vực gần tâm vòng tròn work/home
+ [ ] kiểm tra các luồng tự động vẫn hoạt động tốt với multi-account (schedule, linked tài khoản, generate and upload, history,...) không bị nhầm hoặc xung đột giữa các tài khoản user khác hoặc các phiên cũ trên browser
